...
void ReadFile_king (const string &file_king, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, bool &error, gsl_matrix *G)
{
	igzstream infile (file_king.c_str(), igzstream::in);
	if (!infile) {cout<<"error! fail to open KING file: "<<file_king<<endl; error=true; return;}

	size_t ni_total=indicator_idv.size();

	//can use gsl_matrix_set_identity (G) for prop_ibd_sharing matrix
	gsl_matrix_set_zero (G);

	//set the diagonal to 0.5
	gsl_matrix_diagonal (G);
	gsl_vector_view diag_G = gsl_matrix_diagonal (G);
	gsl_vector_set_all (&diag_G.vector, 0.5);
	...
	while (getline(infile, line)) {
		...
		ch_ptr=strtok (NULL, " , \t");
		id2=ch_ptr;
		ch_ptr=strtok (NULL, " , \t"); //skip n_snp
		ch_ptr=strtok (NULL, " , \t"); //skip HetHet
		ch_ptr=strtok (NULL, " , \t"); //skip IBS0
		ch_ptr=strtok (NULL, " , \t");
		d=atof(ch_ptr);
		if (mapID2num.count(id1)==0 || mapID2num.count(id2)==0) {continue;}
		if (indicator_idv[mapID2num[id1]]==0 || indicator_idv[mapID2num[id2]]==0) {continue;}
...
void ReadFile_seg (const string &file_seg, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, bool &error, gsl_matrix *G)
{
	igzstream infile (file_seg.c_str(), igzstream::in);
	if (!infile) {cout<<"error! fail to open KING file: "<<file_seg<<endl; error=true; return;}
	...
	gsl_matrix_diagonal (G);
	gsl_vector_view diag_G = gsl_matrix_diagonal (G);
	gsl_vector_set_all (&diag_G.vector, 1);
	...
	while (getline(infile, line)) {
		...
		ch_ptr=strtok (NULL, " , \t");
		id2=ch_ptr;
		ch_ptr=strtok (NULL, " , \t"); //skip MaxIBD1
		ch_ptr=strtok (NULL, " , \t"); //skip MaxIBD2
		ch_ptr=strtok (NULL, " , \t"); //skip IBD1Seg
		ch_ptr=strtok (NULL, " , \t"); //skip IBD2Seg
		ch_ptr=strtok (NULL, " , \t");
		d=atof(ch_ptr);
		if (mapID2num.count(id1)==0 || mapID2num.count(id2)==0) {continue;}
		if (indicator_idv[mapID2num[id1]]==0 || indicator_idv[mapID2num[id2]]==0) {continue;}
...
// void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, bool &error, gsl_matrix *G, map <int, map <double,Segments> > &chrpos2segs)
// void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, bool &error, gsl_matrix *G, map <int, map <double, unordered_map <int, unordered_map <int, double> > > > &chrpos2segs)
// void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, const vector<SNPINFO> &snpInfo, bool &error, gsl_matrix *G, map <int, map <long int, unordered_map <int, unordered_map <int, double> > > > &chrpos2segs)
// void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, const vector<SNPINFO> &snpInfo, bool &error, gsl_matrix *G, map <int, map <long int, map <int, map <int, double> > > > &chrpos2segs)
void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, const vector<SNPINFO> &snpInfo, bool &error, gsl_matrix *G, map <int, map <int, FILE *> > &chrpos2segs, const string &f_pref)
// void ReadFile_segments (const string &file_segments, vector<int> &indicator_idv, map<string, int> &mapID2num, const size_t k_mode, const vector<SNPINFO> &snpInfo, bool &error, gsl_matrix *G, map <int, map <long int, sqlite3_stmt*> > &chrpos2segs, sqlite3 *db)
{
	...
	gsl_matrix_diagonal (G);
	gsl_vector_view diag_G = gsl_matrix_diagonal (G);
	gsl_vector_set_all (&diag_G.vector, 2);
	...
	//map rsid to pos
	// map<string, long int> mapRs2pos;
	unordered_map <string, long int> mapRs2pos(snpInfo.size());
	for (vector<SNPINFO>::size_type t=0; t<snpInfo.size(); ++t)
		mapRs2pos[snpInfo[t].rs_number] = snpInfo[t].base_position;
	...
	//for db
	// string queryString;
	// string queryString = "INSERT INTO s VALUES (?1, ?2, ?3, ?4, ?5)";
	// string queryString = "INSERT INTO s VALUES (?1, ?2, ?3, ?4)";
	
	// sqlite3_exec(db, "PRAGMA synchronous = OFF", NULL, NULL, NULL);
	// sqlite3_exec(db, "PRAGMA journal_mode = MEMORY", NULL, NULL, NULL);
	
	// sqlite3_exec(db, "BEGIN TRANSACTION", NULL, NULL, NULL);
	// sqlite3_exec(db, "CREATE TABLE s (chr int, pos int, id1 int, id2 int, ibd double)", NULL, NULL, NULL);
	// sqlite3_exec(db, "CREATE TABLE s (chrpos int, id1 int, id2 int, ibd double)", NULL, NULL, NULL);
	...
	while (getline(infile, line)) {
		...
		//insert code here?
		// chrpos2segs[chr][StartMB].add_seg(n_id1, n_id2, ibd);
		// chrpos2segs[chr][StopMB].add_seg(n_id1, n_id2, 0);
		// chrpos2segs[chr][StartMB][n_id1][n_id2] = ibd;
		// chrpos2segs[chr][StopMB][n_id1][n_id2] = 0;
		// chrpos2segs[chr][StartMB][n_id1].emplace(n_id2, ibd);
		// chrpos2segs[chr][StopMB][n_id1].emplace(n_id2, 0);
		
		StartPos = mapRs2pos.at(StartSNP);
		StopPos = mapRs2pos.at(StopSNP);
		
		StartMB = StartPos / 1000000;
		StopMB = StopPos / 1000000;
		
		// chrpos2segs[chr][StartPos][n_id1][n_id2] = ibd;
		// chrpos2segs[chr][StopPos][n_id1][n_id2] = 0;
		// chrpos2segs[chr][StopPos][n_id1].emplace(n_id2, 0);
		string segs_f_str = f_pref+to_string(chr)+"."+to_string(StartMB);
		if(!chrpos2segs[chr][StartMB]){
		  chrpos2segs[chr][StartMB] = fopen(segs_f_str.c_str(), "wb+");
		  // chrpos2segs[chr][StartMB] = tmpfile();
		  if (!chrpos2segs[chr][StartMB]) {cout<<"error! fail to open file: "<<segs_f_str<<endl; error=true; return;}
		}
		
		fwrite(&StartPos, sizeof(uint32_t), 1, chrpos2segs[chr][StartMB]);
		if (ferror(chrpos2segs[chr][StartMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&n_id1, sizeof(uint32_t), 1, chrpos2segs[chr][StartMB]);
		if (ferror(chrpos2segs[chr][StartMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&n_id2, sizeof(uint32_t), 1, chrpos2segs[chr][StartMB]);
		if (ferror(chrpos2segs[chr][StartMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&ibd, sizeof(uint8_t), 1, chrpos2segs[chr][StartMB]);
		if (ferror(chrpos2segs[chr][StartMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}

		// string segs_f_str = f_pref+to_string(chr)+"."+to_string(StopMB);
		segs_f_str = f_pref+to_string(chr)+"."+to_string(StopMB);
		if(!chrpos2segs[chr][StopMB]){
		  chrpos2segs[chr][StopMB] = fopen(segs_f_str.c_str(), "wb+");
		  // chrpos2segs[chr][StopMB] = tmpfile();
		  if (!chrpos2segs[chr][StopMB]) {cout<<"error! fail to open file: "<<segs_f_str<<endl; error=true; return;}
		}
		fwrite(&StopPos, sizeof(uint32_t), 1, chrpos2segs[chr][StopMB]);
		if (ferror(chrpos2segs[chr][StopMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&n_id1, sizeof(uint32_t), 1, chrpos2segs[chr][StopMB]);
		if (ferror(chrpos2segs[chr][StopMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&n_id2, sizeof(uint32_t), 1, chrpos2segs[chr][StopMB]);
		if (ferror(chrpos2segs[chr][StopMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		fwrite(&zero, sizeof(uint8_t), 1, chrpos2segs[chr][StopMB]);
		if (ferror(chrpos2segs[chr][StopMB])) {cout<<"error! fail to write to file: "<<segs_f_str<<endl; error=true; return;}
		
		//ibd for start of segment
		// chrpos2segs[chr][StartPos];
		// if(!chrpos2segs[chr][StartPos]){
			//create table
			// queryString = string("CREATE TABLE chr") + 
				// to_string(chr) + "_" + to_string(StartPos) + 
				// " (id1 int, id2 int, ibd double)";
			// queryString = string("CREATE TABLE chr") + to_string(chr) + 
				// " (pos int, id1 int, id2 int, ibd double)";
			// sqlite3_stmt* queryStmt = NULL;
			// if (sqlite3_prepare_v2(db, queryString.c_str(), -1, &queryStmt, NULL) != SQLITE_OK) {
				// cerr << "Failed to prepare statement" << endl;
				// return;
			...
			// queryString = string("INSERT INTO chr") + 
				// to_string(chr) + "_" + to_string(StartPos) + 
				// " VALUES (?1, ?2, ?3)";
			// if (sqlite3_prepare_v2(db, queryString.c_str(), -1, &chrpos2segs[chr][StartPos], NULL) != SQLITE_OK) {
				// cerr << "Failed to prepare statement" << endl;
				// return;
			// }
		// }
		...
		// if (	sqlite3_bind_int(chrpos2segs[chr][StartPos], 1, n_id1) != SQLITE_OK ||
				// sqlite3_bind_int(chrpos2segs[chr][StartPos], 2, n_id2) != SQLITE_OK ||
				// sqlite3_bind_double(chrpos2segs[chr][StartPos], 3, ibd) != SQLITE_OK	) {
				// cerr << "Failed to bind" << endl;
			// return;
		// }
		// if (	//sqlite3_bind_int(queryStmt, 1, chr) != SQLITE_OK ||
				//sqlite3_bind_int(queryStmt, 2, StartPos) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 1, chr * 1000000000 + StartPos) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 2, n_id1) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 3, n_id2) != SQLITE_OK ||
				// sqlite3_bind_double(queryStmt, 4, ibd) != SQLITE_OK	) {
				// cerr << "Failed to bind" << endl;
			// return;
		// }
		// sqlite3_step(chrpos2segs[chr][StartPos]);
		// sqlite3_clear_bindings(chrpos2segs[chr][StartPos]);
		// sqlite3_reset(chrpos2segs[chr][StartPos]);
		// sqlite3_step(queryStmt);
		// sqlite3_clear_bindings(queryStmt);
		// sqlite3_reset(queryStmt);

		//ibd for end of segment
		// chrpos2segs[chr][StopPos];
		// if(!chrpos2segs[chr][StopPos]){
			//create table
			// queryString = string("CREATE TABLE chr") + 
				// to_string(chr) + "_" + to_string(StopPos) + 
				// " (id1 int, id2 int, ibd double)";
			// sqlite3_stmt* queryStmt = NULL;
			// if (sqlite3_prepare_v2(db, queryString.c_str(), -1, &queryStmt, NULL) != SQLITE_OK) {
				// cerr << "Failed to prepare statement" << endl;
			...
			// queryString = string("INSERT INTO chr") + 
				// to_string(chr) + "_" + to_string(StopPos) + 
				// " VALUES (?1, ?2, ?3)";
			// if (sqlite3_prepare_v2(db, queryString.c_str(), -1, &chrpos2segs[chr][StopPos], NULL) != SQLITE_OK) {
				// cerr << "Failed to prepare statement" << endl;
				// return;
			// }
		// }
		...
		//bind parameter values
		// if (	sqlite3_bind_int(chrpos2segs[chr][StopPos], 1, n_id1) != SQLITE_OK ||
				// sqlite3_bind_int(chrpos2segs[chr][StopPos], 2, n_id2) != SQLITE_OK ||
				// sqlite3_bind_double(chrpos2segs[chr][StopPos], 3, 0) != SQLITE_OK	) {
				// cerr << "Failed to bind" << endl;
			// return;
		// }
		// if (	//sqlite3_bind_int(queryStmt, 1, chr) != SQLITE_OK ||
				//sqlite3_bind_int(queryStmt, 2, StopPos) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 1, chr * 1000000000 + StopPos) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 2, n_id1) != SQLITE_OK ||
				// sqlite3_bind_int(queryStmt, 3, n_id2) != SQLITE_OK ||
				// sqlite3_bind_double(queryStmt, 4, 0) != SQLITE_OK	) {
				// cerr << "Failed to bind" << endl;
			// return;
		// }

		// sqlite3_step(chrpos2segs[chr][StopPos]);
		// sqlite3_clear_bindings(chrpos2segs[chr][StopPos]);
		// sqlite3_reset(chrpos2segs[chr][StopPos]);
		// sqlite3_step(queryStmt);
		// sqlite3_clear_bindings(queryStmt);
		// sqlite3_reset(queryStmt);
	...
	//todo:
	//one table for each chr
	//index on pos column
	// sqlite3_finalize(queryStmt);

	// sqlite3_exec(db, "CREATE INDEX chr_pos ON s (chr, pos) ", NULL, NULL, NULL);
	// sqlite3_exec(db, "CREATE INDEX chr_pos ON s (chrpos) ", NULL, NULL, NULL);
	// sqlite3_exec(db, "END TRANSACTION", NULL, NULL, NULL);

// map <int, map <double,Segments> >::iterator it = chrpos2segs.begin();
// cout << (++it)->second.second.id1[0] << endl;
	// while (chrpos2segs[1][0.706].get_seg(&n_id1, &n_id2, &ibd)) {
	// 	// Cov_d=gsl_matrix_get(G, n_id1, n_id2);
...
void WriteFile_combined_kin (PARAM &cPar, gsl_matrix *G)
{
  map<size_t, size_t> mapID2ID;
  size_t c=0;
  ...
  for (map<string, int>::iterator it1 = cPar.mapID2num.begin(); it1!=cPar.mapID2num.end(); ++it1) {
	...
	for (map<string, int>::iterator it2 = it1; it2!=cPar.mapID2num.end(); ++it2) {
	  ...
	  for (int vc=0; vc < n_vc; vc++) {
		g += cPar.v_sigma2[vc] * gsl_matrix_get(G, n_id1, n1*vc + n_id2);
	  }
