...
void PARAM::ReadFiles (void)
{
	...
	*/
	...
	if (!file_mk.empty()) {
	  ...
	  if (!file_segments.empty()) {n_vc++;}
	}

	//GJMZ Added: code to read king files
	// else if (!file_mk.empty() && !file_segments.empty()){
	  // n_vc++;
	// }
	else if (!file_kin.empty() && !file_segments.empty()){
	  n_vc = 2;
	}
	else if (!file_king.empty() && !file_segments.empty()){
	  n_vc = 2;
	}
	else if (!file_seg.empty() && !file_segments.empty()){
	  n_vc = 2;
	}
