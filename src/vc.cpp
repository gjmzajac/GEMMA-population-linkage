...
cout << "y_scale" << endl;
gsl_vector_fprintf(stdout, y_scale, "%f");
cout << "K" << endl;
gsl_matrix_fprintf(stdout, K, "%f");
cout << "K_scale" << endl;
gsl_matrix_fprintf(stdout, &Kscale_sub.matrix, "%f");
cout << "r" << endl << r <<endl;
cout << "Kry" << endl;
gsl_vector_fprintf(stdout, &Kry_col.vector, "%f");
debug */
  }
...
/* debug
cout << "yKrKKry" << endl;
gsl_matrix_fprintf(stdout, yKrKKry, "%f");
debug */
...
/* debug
cout << "qvar_mat" << endl;
gsl_matrix_fprintf(stdout, qvar_mat, "%f");
debug */
...
//GJMZ added to fit 2 VC model for linkage faster
// void VC::CalcVChe (const gsl_matrix *K, const gsl_matrix *W, const gsl_vector *y)
// void VC::Calc2VCheMulti (const gsl_matrix *K_scale, vector<double> &traceG_new, const gsl_vector *y_scale, double var_y, double var_y_new, gsl_matrix * &Kry, gsl_vector * &q_vec, gsl_matrix * &yKrKKry, gsl_matrix * &S_mat, double &s_11, double &s_12, double &s_22)
void VC::Calc2VCheMulti (const gsl_matrix *K_scale, vector<double> &traceG_new, const gsl_vector *y_scale, double var_y, double var_y_new, gsl_matrix * &Kry, gsl_vector * &q_vec, gsl_matrix * &yKrKKry, gsl_matrix * &S_mat)
{
  // size_t n1=K->size1, n2=K->size2;
  size_t n1=K_scale->size1, n2=K_scale->size2;
  size_t n_vc=n2/n1;

  double r=(double)n1/(double)(n1 - 1);
  // double var_y, var_y_new;
  double d, tr, s, v;
  // vector<double> traceG_new;
  bool first_vc=false;
  ...
  // for (size_t i=0; i<n_vc; i++) {
    ...
    //testing without this function. Can make conditional later.
    // CenterMatrix (&Kscale_sub.matrix, W);
    // d=ScaleMatrix (&Kscale_sub.matrix);
    // traceG_new.push_back(d);
  // }
  ...
  //compute Kry, which is used for confidence interval; also compute q_vec (*n^2)
  if (Kry == NULL){
	Kry=gsl_matrix_alloc (n1, n_vc);
	first_vc=true;
  }
  for (size_t i=0; i<n_vc; i++) {
	if (first_vc || i==n_vc-1){
	  gsl_matrix_const_view Kscale_sub = gsl_matrix_const_submatrix (K_scale, 0, n1*i, n1, n1);
	  gsl_vector_view Kry_col=gsl_matrix_column (Kry, i);
...
debug */
  ...
  // for (size_t i=0; i<n_vc; i++) {
    ...
    // for (size_t j=i; j<n_vc; j++) {
      // gsl_vector_const_view Kry_colj=gsl_matrix_const_column (Kry, j);
	  // if (yKrKKry == NULL)
		// yKrKKry=gsl_matrix_alloc (n_vc, n_vc*(n_vc+1) );
	  // else if (i==0 && j==0){
		// l=1;
		// goto label;
	  // }
  ...
  for (size_t i=0; i<n_vc; i++) {
    ...
    for (size_t j=i; j<n_vc; j++) {
      gsl_vector_const_view Kry_colj=gsl_matrix_const_column (Kry, j);
      for (size_t l=0; l<n_vc; l++) {
		if (first_vc || i==n_vc-1 || j==n_vc-1 || l==n_vc-1) {
		  gsl_matrix_const_view Kscale_sub = gsl_matrix_const_submatrix (K_scale, 0, n1*l, n1, n1);
		  gsl_blas_dgemv (CblasNoTrans, 1.0, &Kscale_sub.matrix, &Kry_coli.vector, 0.0, KKry);
		  gsl_blas_ddot (&Kry_colj.vector, KKry, &d);
		  gsl_matrix_set(yKrKKry, i, l*n_vc+j, d);
...
debug */
  ...
  //compute Sij (*n^2)
  if (S_mat == NULL){
	S_mat = gsl_matrix_alloc (n_vc, n_vc);
	first_vc=true;
  }
  for (size_t i=0; i<n_vc; i++) {
    for (size_t j=i; j<n_vc; j++) {
  ...
  */
  ...
  //print s_12, s_22 to output file
  // s_12 = gsl_matrix_get(S_mat, 0, n_vc-1);
  // s_22 = gsl_matrix_get(S_mat, n_vc-1, n_vc-1);

  //save s_11
  // double s_11 = gsl_matrix_get (S_mat, 0, 0);
  // s_11 = gsl_matrix_get (S_mat, 0, 0);
  gsl_matrix *S_mat_tmp = gsl_matrix_alloc (n_vc-1, n_vc-1);
  gsl_matrix_view S_mat_sub=gsl_matrix_submatrix(S_mat, 0, 0, n_vc-1, n_vc-1);
  gsl_matrix_memcpy(S_mat_tmp, &S_mat_sub.matrix);
  ...
  //reuse s_11 on the next iteration
  // gsl_matrix_set (S_mat, 0, 0, s_11);
  gsl_matrix_memcpy(&S_mat_sub.matrix, S_mat_tmp);
  gsl_matrix_free(S_mat_tmp);
